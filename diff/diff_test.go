...
func TestDiff(t *testing.T) {
	...
	specs := []Spec{
		...
		// not change on update current_timestamp(case insensitive)
		{
			Before: "CREATE TABLE `fuga` ( `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP );",
			After:  "CREATE TABLE `fuga` ( `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE current_timestamp );",
			Expect: "",
		},
